...
//  Tracker
//
//  Created by Artem Adiev on 30.10.2023.
//
...
extension FiltersViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        // Если у нас уже есть выбранная ячейка, сбрасываем её изображение
        if let prevIndexPath = selectedIndexPath, let prevCell = tableView.cellForRow(at: prevIndexPath) as? FiltersCell {
            prevCell.done(with: UIImage())  // или любое другое изображение
        }
        ...
        if let cell = tableView.cellForRow(at: indexPath) as? FiltersCell {
            ...
            // Обновите трекеры на основе выбранного фильтра
            switch filter {
...
extension FiltersViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // Задаем текст для titleLabel в ячейке
        cell.titleLabel.text = filters[indexPath.row]
        
        // Задаем кнопкам-ячейкам шрифт
        cell.textLabel?.font = UIFont.systemFont(ofSize: 17)
        
        // Проверяем является ли текущая ячейка выбранной
        if indexPath == selectedIndexPath {
            cell.done(with: UIImage(named: "CheckMark") ?? UIImage())
        } else {
            cell.done(with: UIImage())  // или любое другое изображение
        }
